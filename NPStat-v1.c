...
int main(int argc, char *argv[])
{
    ...
    for(rd=2*m_bar+1;rd<=max_cov;rd++)
    {
        ...
        for(k=1;k<n01;k++)
        {
            comb1.d_t[rd-1]+=(1-gsl_pow_int((double)k/(double)n01,(int)rd)-gsl_pow_int(1-(double)k/(double)n01,(int)rd))/(double)k;
            for(lt=1;lt<=m_bar;lt++)
            {
                comb1.d_t[(int)rd-1]+=-gsl_sf_choose((int)rd,lt)*gsl_pow_int((double)k/(double)n01,lt-1)*gsl_pow_int(1-(double)k/(double)n01,(int)rd-lt-1)/(double)n01;
                comb1.d_p[(int)rd-1]+=-2.0*gsl_sf_choose((int)rd-2,lt-1)*gsl_pow_int((double)k/(double)n01,lt-1)*gsl_pow_int(1-(double)k/(double)n01,(int)rd-lt-1)/(double)n01;
                comb1.d_hl[(int)rd-1]+=-(double)rd/(double)(n01*(rd-1))* gsl_sf_choose((int)rd-1,lt-1)*gsl_pow_int((double)k/(double)n01,lt-1)*gsl_pow_int(1-(double)k/(double)n01,(int)rd-lt) -(double)rd/(double)(n01*(rd-1))* gsl_sf_choose((int)rd-1,(int)rd-lt-1)*gsl_pow_int((double)k/(double)n01,(int)rd-lt-1)*gsl_pow_int(1-(double)k/(double)n01,lt);
                comb1.d_hq[(int)rd-1]+=-1.0/(double)(n01*(rd-1))*((double)lt*gsl_sf_choose((int)rd-1,lt-1)*gsl_pow_int((double)k/(double)n01,lt-1)*gsl_pow_int(1-(double)k/(double)n01,(int)rd-lt) +(double)(rd-lt)*gsl_sf_choose((int)rd-1,(int)rd-lt-1)*gsl_pow_int((double)k/(double)n01,(int)rd-lt-1)*gsl_pow_int(1-(double)k/(double)n01,lt));
            };
            comb1.d_hl[(int)rd-1]+=-(double)rd/(double)(n01*(rd-1))*(gsl_pow_int((double)k/(double)n01,(int)rd-1));
            //comb1.d_hl[rd-1]+=((double)rd/(double)n01)*((1-2/((double)rd-1))*(double)k/(double)n01-1)*gsl_pow_int((double)k/(double)n01,rd-2);
            comb1.d_hq[(int)rd-1]+=-(double)rd/(double)(n01*(rd-1))*(gsl_pow_int((double)k/(double)n01,(int)rd-1));
    ...
    for(rd=2*m_bar+1;rd<=max_cov; rd++){
        ...
        vec0_h[rd-1]=0;
        for(k=1;k<n01;k++){
            vec_s[(k-1)+(n01-1)*(rd-1)]=(1-gsl_pow_int((double)k/(double)n01,(int)rd)-gsl_pow_int(1-(double)k/(double)n01,(int)rd));
            vec_p[(k-1)+(n01-1)*(rd-1)]=(double)(2*k*(n01-k))/(double)(n01*n01);
            vec_h[(k-1)+(n01-1)*(rd-1)]=(double)(k*k)/(double)(n01*n01)+(double)(k)/(double)(n01*(rd-1))-(double)rd/(double)(rd-1)*gsl_pow_int((double)(k)/(double)(n01),(int)rd);
            for(covt=1;covt<=m_bar;covt++){
                vec_s[(k-1)+(n01-1)*((int)rd-1)]+=-( gsl_ran_binomial_pdf(covt,	(double)k/(double)n01,(int)rd)+gsl_ran_binomial_pdf((int)rd-covt,(double)k/(double)n01,(int)rd));
                vec_p[(k-1)+(n01-1)*(rd-1)]+=-( 2*(double)(covt*(rd-covt))/(double)(rd*(rd-1))*(gsl_ran_binomial_pdf(covt,	(double)k/(double)n01,(int)rd)+gsl_ran_binomial_pdf((int)rd-covt,(double)k/(double)n01,(int)rd)));
                vec_h[(k-1)+(n01-1)*(rd-1)]+=-( (double)(covt*covt)/(double)(rd*(rd-1))*gsl_ran_binomial_pdf(covt,	(double)k/(double)n01,(int)rd)+(double)(rd-covt)*(double)(rd-covt)/(double)(rd*(rd-1))*gsl_ran_binomial_pdf((int)rd-covt,(double)k/(double)n01,(int)rd));
            };
            for(covt=m_bar+1;covt<=rd-m_bar-1;covt++){
                vec0_s[rd-1]+=gsl_ran_binomial_pdf(covt,(double)k/(double)n01,(int)rd)/k;
                vec0_d[rd-1]+=gsl_pow_int(2*(double)(covt*(rd-covt))/(double)(rd*(rd-1))/comb1.d_p[rd-1]-1/comb1.d_t[rd-1],2)*gsl_ran_binomial_pdf(covt,(double)k/(double)n01,(int)rd)/k;
                vec0_h[rd-1]+=gsl_pow_int((double)(covt*covt)/(double)(rd*(rd-1))/comb1.d_hq[rd-1]-2*(double)(covt*(rd-covt))/(double)(rd*(rd-1))/comb1.d_p[rd-1],2)*gsl_ran_binomial_pdf(covt,(double)k/(double)n01,(int)rd)/k;
            };
        };
    };
    ...
    for(pos=1;(ct1!=EOF); pos++)
    {
        ...
        if (pos_base1<pos)
        {
            ...
            if(read_line_pileup(bam_file1, min_qual, min_mqual, &pos_base1, &n_ref1, &n_alt_allele1, &rd1, n_alt_1, &ref_base1, &alt_base1)==1) {
                printf("Exiting file\n");
                ct1=EOF;
            }
            //  read_line_ms(bam_file1, window_size, n01, &n_pos1, int_positions1, array_counts1);
        ...
        if ((pos==(n_window*window_size))||(ct1==EOF))
        {
            ...
            if((test1.den_p>0)&&(test1.den_hq>0)) { h1_val=pi1_val-test1.num_hq/test1.den_hq; } else { h1_val=-1; };
